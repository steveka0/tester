...
jqlSearch = "'Epic Link' = " + issue.getKey() + " and issuetype != 'Core Crate Planning Task' and 'Locked In Product' = 'Yes'"
...
for(int i=0; i < issues.size(); i++)
{
	...
	{
    	if (cFields.get(x).getFieldName() == "Cost/Unit")
    	{
     	    unitCost = cFields.get(x)
			dValue = (Double)item.getCustomFieldValue(unitCost)
            if (dValue != null)
           		totalUnitCost = totalUnitCost + dValue.doubleValue()
        }
 	}
